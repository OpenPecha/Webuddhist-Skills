...
{{
  ...
  "Glossary": {{ // Obsidian **Interlinear Glossing** (ling-gloss) content for a ```gloss``` code block. Token counts on gla, glb, and glc MUST match (same number of space-separated gloss elements). Gloss the **authoritative** reading (after applying StandardizedText if any).
    "gla": "String", // Source line: space-separated tokens (Tibetan script, EWTS/Wylie, IAST, or romanisation as appropriate).
    "glb": "String", // WSD in **source language**: unambiguous tokens repeat the token/lemma without parentheses; ambiguous tokens use token(selected-sense-in-source-language); compounds show analysis, e.g. sa-gaṇa(saha-gaṇa).
    "glc": "String", // English gloss line: plain gloss per token; where commentary disambiguates, use plain-gloss(English-sense) matching Wikipedia-style sense IDs; parentheses ONLY when commentary picks a specific sense.
    "free_translation": "String", // One full-sentence English rendering of the verse from traditional interpretation (exported as ling-gloss \\ft).
    "source_citation": "String", // Optional vault-relative path or block ref (e.g. 1-Human-Sources/...#^id); empty if unknown.
    "divergences": "String", // Optional: when commentaries gloss a token differently, markdown lines (no section heading) documenting alternatives, each flagged with ⚑; primary reading stays in glb/glc; empty if none.
    "glossary": "String" // Legacy bullet glossary; normally empty "" — do not use unless interlinear fields cannot be produced.
  }}
}}
```
...
**Stage 3: Populate the `Glossary` Object (ling-gloss interlinear)**
1.  Split the **authoritative** verse into the same meaningful units on **`gla`**, **`glb`**, and **`glc`** (equal element counts; use `[bracketed phrases]` for a single element that contains spaces, per ling-gloss rules).
2.  **`glb`**: In the **same language/script family as the source** (e.g. Tibetan for Tibetan source), encode word-sense disambiguation; repeat unambiguous tokens; use `token(sense)` only when needed.
3.  **`glc`**: English aligned glosses; add `(sense)` only for commentary-driven disambiguation.
4.  **`free_translation`**: Fluent English of the whole line, consistent with commentaries and UCCA.
5.  **`divergences`**: If commentaries disagree on a gloss, keep the primary reading in **`glb`/`glc`** and list alternatives here with ⚑.
6.  **`glossary`**: Leave `""` unless you cannot align interlinear lines.
...
{{
  ...
  "Glossary": {{
    "gla": "དམ་པའི་ཆོས་ ནི་ བསྟན་པར་ ཤོག",
    "glb": "དམ་པའི་ཆོས་ ནི་ བསྟན་པར་(བསྟན་པའི་དོན།) ཤོག",
    "glc": "holy-Dharma topic to-be-taught(teaching-of-dharma) may-it-be",
    "free_translation": "May the holy Dharma be taught — i.e. may there be the exposition of the true Dharma, in line with the Sanskrit saddharma-deśanā.",
    "source_citation": "",
    "divergences": "- ⚑ **བསྟན་པར་ vs བཟང་བར་**: Commentary 1 supports the reading 'good' (བཟང་པོ་) aligned with the surface བཟང་བར་; Commentary 2 and Sanskrit deśanā support 'teach' — adopted as primary above.",
    "glossary": ""
  }}
}}
```
...
async def stream_gloss_generation(
    ...
    for idx, it in enumerate(items):
        ...
        if data:
            try:
                gdict = data.get("Glossary") or {}
                glossary = gdict.get("glossary")
                ...
                item_payload = {
                    ...
                    "glb": gdict.get("glb"),
                    "glc": gdict.get("glc"),
                    "free_translation": gdict.get("free_translation"),
                    "source_citation": gdict.get("source_citation"),
                    "divergences": gdict.get("divergences"),
                }
